...
import { atomicWriteFile, canonicalize, confineToRoot, containingRoot, DIR_CREATE_MODE, fsErrorMessage, isWithin, } from "./fs-util.mjs";
export { setupSkills, resolveSkillVersion, extractSkillArchive } from "./skills.mjs";
export { SessionMemoryStores, SessionMemoryError, DEFAULT_MEMORY_SYNC_INTERVAL_MS, MIN_MEMORY_SYNC_INTERVAL_MS, MEMORY_FLUSH_TIMEOUT_MS, MARKER_PATH, } from "./memories.mjs";
...
function rejectUnrestrictedPaths(value) {
    if (value === undefined)
        return;
    throw new AnthropicError('The `unrestrictedPaths` option you passed to the agent toolset (AgentToolContext) is no longer ' +
